import { BadRequestException, Body, Controller, Delete, Get, Inject, Param, Post, Patch, Put, UploadedFile, UseInterceptors, Query } from '@nestjs/common';
...
export class PetController {
    ...
    private readonly getPetByIdUseCase:
        IUseCase<GetPetByIdUseCaseInput, GetPetByIdUseCaseOutput>

    @Inject(PetTokens.updatePetByIdUseCase)
    private readonly updatePetByIdUseCase:
    IUseCase<UpdatePetByIdUseCaseInput, UpdatePetByIdUseCaseOutput>
    ...
    async getPets(
        ...
        @Query('itemPerPage') itemsPerPage?: string,
    ){
        const FIRST_PAGE = 1
    ...
    @Get(':id')
    async getPeById(@Param('id') id: string): Promise<GetPetByIdUseCaseOutput> {
        try {
